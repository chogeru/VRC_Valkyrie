    ...
    public class MAIAnimalLink : MonoBehaviour
    {
        ...
        private void Reset()
        {
            if (!TryGetComponent<Unity.AI.Navigation.NavMeshLink>(out var navLink))
            {
                ...
                // NavMeshLink.startTransform/endTransform are com.unity.ai.navigation 2.x-only
                // (requires Unity 6's UnityEngine.AI.NavMesh link APIs). This project pins
                // ai.navigation 1.1.5 for VRChat SDK/Unity 2022.3 compatibility, so fall back
                // to the 1.x Vector3 point API instead.
                navMeshLink.startPoint = navMeshLink.transform.InverseTransformPoint(start.position);
                navMeshLink.endPoint = navMeshLink.transform.InverseTransformPoint(end.position);
            }
