...
#else
            this.Delay_Action(1, () => Brain.m_WorldUpOverride = UpVector);
#endif
...
#if UNITY_6000_0_OR_NEWER
                bool lateUpdateBrain = Brain.UpdateMethod == CinemachineBrain.UpdateMethods.LateUpdate;
#else
                bool lateUpdateBrain = Brain.m_UpdateMethod == CinemachineBrain.UpdateMethod.LateUpdate;
#endif
                if (lateUpdateBrain)
                {
...
#else
            if (ThisCamera is CinemachineVirtualCameraBase cam)
            {
                cam.Priority = value ? priority : -1;
            }
#endif
        }
