...
namespace MalbersAnimations.Weapons
{
    ...
    public class MProjectile : MDamager, IProjectile
    {
        ...
        protected virtual IEnumerator RollingProjectile()
        {
            ...
            Vector3 prev_velocity = rb != null ? rb.velocity : Velocity; // Initial velocity for the projectile
            ...
            while (!HasImpacted && enabled)
            {
                ...
                // Projectile stopped
                if (rb.velocity.sqrMagnitude < 0.00001f && TraveledDistance > 1)
                {
                ...
                if (rb.velocity.sqrMagnitude > 0.001f && prev_velocity.sqrMagnitude > 0.001f)
                {
                    float currentDeflection = Vector3.Angle(Vector3.ProjectOnPlane(prev_velocity, Vector3.up), Vector3.ProjectOnPlane(rb.velocity, Vector3.up));
                ...
                {
                    T.rotation = Quaternion.LookRotation(rb.velocity.normalized, T.up);
                    if (TrajectoryRoll != 0) T.Rotate(rb.velocity.normalized, TrajectoryRoll * Time.fixedDeltaTime, Space.World);
                }
